...
int create_new_stack(void){
  stack* s;
  if((s=(stack*)malloc(sizeof(stack)))==NULL) return 1;
  s->Snext=the_stack;
  occstack=0;
  the_stack=s;
  stack_depth++;
...
int get(int *val){
  if(occstack==0){
    if(the_stack==NULL || the_stack->Snext==NULL) return 1;
    *val=the_stack->v_stack[STACK_SIZE-1];
  }
  *val=the_stack->v_stack[occstack-1];
  return 0;
}
...
int dirload(int i, int *val){
  ...
  if(s==NULL) return 1;
  if(st==stack_depth && i%STACK_SIZE>=occstack) return 1;
...
int display_stack(void){
  ...
  while(s!=NULL){
    for(;i>=0;i--){
      printf("%d\n",s->v_stack[i]);
      if(base==st*STACK_SIZE+i) printf("____\n");
    }
    i=STACK_SIZE-1;
    s=s->Snext;
    st--;
